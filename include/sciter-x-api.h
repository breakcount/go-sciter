...
#endif
  BOOL    SCFN( SciterLoadFile )(HWINDOW hWndSciter, LPCWSTR filename);//载入文件

  BOOL    SCFN( SciterLoadHtml )(HWINDOW hWndSciter, LPCBYTE html, UINT htmlSize, LPCWSTR baseUrl);//载入HTML
  VOID    SCFN( SciterSetCallback )(HWINDOW hWndSciter, LPSciterHostCallback cb, LPVOID cbParam);//设置回调
...
#if defined(WINDOWS)
  BOOL    SCFN( SciterRenderD2D )(HWINDOW hWndSciter, IUnknown* /*ID2D1RenderTarget**/ prt);//渲染D2D
...
#endif
  HWINDOW SCFN( SciterCreateWindow )( UINT creationFlags,
                                      LPRECT frame, 
...
//|
  ...
  HSARCHIVE SCFN( SciterOpenArchive ) (LPCBYTE archiveData, UINT archiveDataLength);//打开档案
...
inline ISciterAPI *_SAPI(ISciterAPI *ext) {
  ...
  // defining "official" API functions:  定义 "正式 "的API功能。 是实现
  ...
  inline BOOL  SCAPI SciterAtomNameCB(UINT64 atomv, LPCSTR_RECEIVER* rcv, LPVOID rcv_param) { return SAPI()->SciterAtomNameCB(atomv, rcv, rcv_param); }
  inline BOOL  SCAPI SciterSetGlobalAsset(som_asset_t* pass) { return SAPI()->SciterSetGlobalAsset(pass); }
  inline BOOL  SCAPI SciterReleaseGlobalAsset(som_asset_t* pass) { return SAPI()->SciterReleaseGlobalAsset(pass); }
